...
# Pruned for the diffuse-rocm engine: the upstream file imports the dataset/ and flux/ packages
# (for control-image bucketing and fp8 detection). This engine is bf16-only text2image, so:
#   * `is_fp8` is replaced with a constant False (no fp8 path is ever taken),
#   * `BucketSelector` / `image_utils` (used only by the dead `preprocess_control_image` helper)
#     are dropped entirely. `preprocess_control_image` is never called for text2image.
